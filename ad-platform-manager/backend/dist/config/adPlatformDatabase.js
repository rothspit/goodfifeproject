...
exports.initializeAdPlatformTables = initializeAdPlatformTables;
exports.setupAdPlatformDatabase = setupAdPlatformDatabase;
const database_1 = require("./database");
/**
 * 広告媒体統合管理システム用データベーステーブル初期化
 */
async function initializeAdPlatformTables() {
...
async function insertInitialPlatforms() {
    try {
        // シティヘブンネットの存在確認
        const [cityHeaven] = await database_1.pool.execute('SELECT id FROM ad_platforms WHERE name = ?', ['シティヘブンネット']);
      ...
      `, [
        ...
        // デリヘルタウンの存在確認
        const [deliheruTown] = await database_1.pool.execute('SELECT id FROM ad_platforms WHERE name = ?', ['デリヘルタウン']);
 ...
 */
async function setupAdPlatformDatabase() {
    try {
...
//# sourceMappingURL=adPlatformDatabase.js.map