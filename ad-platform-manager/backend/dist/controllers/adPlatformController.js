...
exports.getDistributionStatistics = exports.getDistributionLogs = exports.deletePlatform = exports.updatePlatform = exports.createPlatform = exports.getPlatformById = exports.getAllPlatforms = void 0;
exports.getDecryptedPlatformCredentials = getDecryptedPlatformCredentials;
...
const getAllPlatforms = async (req, res) => {
    try {
        ...
        const [platforms] = await database_1.pool.execute(query, params);
...
const getPlatformById = async (req, res) => {
    try {
        const { id } = req.params;
        const [platforms] = await database_1.pool.execute('SELECT * FROM ad_platforms WHERE id = ?', [id]);
...
const createPlatform = async (req, res) => {
    try {
        const { name, category, url, login_id, login_password, connection_type, api_endpoint, api_key, api_secret, ftp_host, ftp_port, ftp_username, ftp_password, is_active, settings } = req.body;
        if (!name || !category) {
...
const updatePlatform = async (req, res) => {
    try {
        ...
        const { name, category, url, login_id, login_password, connection_type, api_endpoint, api_key, api_secret, ftp_host, ftp_port, ftp_username, ftp_password, is_active, settings } = req.body;
        // 既存データを確認
        const [existing] = await database_1.pool.execute('SELECT id FROM ad_platforms WHERE id = ?', [id]);
        ...
        await database_1.pool.execute(`UPDATE ad_platforms SET ${updateFields.join(', ')} WHERE id = ?`, updateValues);
        res.json({
            success: true,
            message: '媒体情報を更新しました'
        });
...
const deletePlatform = async (req, res) => {
    try {
        const { id } = req.params;
        const [result] = await database_1.pool.execute('DELETE FROM ad_platforms WHERE id = ?', [id]);
 ...
 * 配信ログ一覧取得
 */
const getDistributionLogs = async (req, res) => {
    try {
    ...
    `;
        ...
        const [logs] = await database_1.pool.execute(query, params);
        res.json({
            success: true,
            logs,
            count: logs.length
...
exports.getDistributionLogs = getDistributionLogs;
/**
 * 配信ログ統計取得
 */
const getDistributionStatistics = async (req, res) => {
    try {
    ...
    `);
        const [platformStats] = await database_1.pool.execute(`
      SELECT 
        ap.name as platform_name,
        COUNT(*) as count,
...
exports.getDistributionStatistics = getDistributionStatistics;
/**
 * パスワード復号化（内部使用）
 */
async function getDecryptedPlatformCredentials(platformId) {
    try {
        const [platforms] = await database_1.pool.execute('SELECT * FROM ad_platforms WHERE id = ?', [platformId]);
        if (platforms.length === 0) {
...
//# sourceMappingURL=adPlatformController.js.map