...
exports.bulkDistribute = exports.distributeDiary = exports.distributeSchedule = exports.distributeCastInfo = void 0;
const database_1 = require("../config/database");
const adPlatformController_1 = require("./adPlatformController");
const HeavenNetService_1 = require("../services/platforms/HeavenNetService");
/**
 * キャスト情報を指定媒体に配信
 */
const distributeCastInfo = async (req, res) => {
    try {
        ...
        // キャスト情報を取得
        const [casts] = await database_1.pool.execute('SELECT * FROM casts WHERE id = ?', [castId]);
        if (casts.length === 0) {
        ...
        for (const platformId of platformIds) {
            ...
            try {
                // 媒体の認証情報を取得
                const platform = await (0, adPlatformController_1.getDecryptedPlatformCredentials)(platformId);
                ...
                switch (platform.name) {
                    case 'シティヘブンネット':
                        // Heaven Net配信
                        const loginSuccess = await HeavenNetService_1.heavenNetService.login({
                            username: platform.login_id,
                            password: platform.login_password
                        });
                        if (loginSuccess) {
                            success = await HeavenNetService_1.heavenNetService.updateCastInfo({
                                id: cast.id,
                                name: cast.name,
                                age: cast.age,
                                height: cast.height,
                            ...
                            await HeavenNetService_1.heavenNetService.logout();
                            await HeavenNetService_1.heavenNetService.close();
                        }
                        break;
                    case 'デリヘルタウン':
                        // Deliheru Town配信
...
exports.distributeCastInfo = distributeCastInfo;
/**
 * スケジュールを指定媒体に配信
 */
const distributeSchedule = async (req, res) => {
    try {
            ...
            try {
                ...
                switch (platform.name) {
                    case 'シティヘブンネット':
                        ...
                        if (loginSuccess) {
                            success = await HeavenNetService_1.heavenNetService.updateSchedule(schedules);
                            await HeavenNetService_1.heavenNetService.logout();
                            await HeavenNetService_1.heavenNetService.close();
                        }
                        break;
...
exports.distributeSchedule = distributeSchedule;
/**
 * 写メ日記を指定媒体に配信
 */
const distributeDiary = async (req, res) => {
    try {
            ...
            try {
                ...
                switch (platform.name) {
                    case 'シティヘブンネット':
                        ...
                        if (loginSuccess) {
                            success = await HeavenNetService_1.heavenNetService.postDiary(castId, title, content, images);
                            await HeavenNetService_1.heavenNetService.logout();
                            await HeavenNetService_1.heavenNetService.close();
                        }
                        break;
...
exports.distributeDiary = distributeDiary;
/**
...
const bulkDistribute = async (req, res) => {
    try {
        ...
        // すべてのアクティブなキャストを取得
        const [casts] = await database_1.pool.execute('SELECT * FROM casts WHERE is_active = 1');
...
exports.bulkDistribute = bulkDistribute;
